...
import AddNewRowView from "./AddNewRowView";
...
const CustomTableObjectView: React.FC<ICustomTableObjectViewProps> = (props) => {
   ...
   const [showAddView, setShowAddView] = React.useState(false);
   React.useEffect(() => {
      const onload = () => {
   ...
   return (
      <RenderIf renderIf={data && data.length > 0}>
                        ...
                        Object.keys(data[0]).map((key, keyIndex) => {
                           return (
                              <th key={keyIndex}>
                                 ...
                                 >
                                    <Button
                                       icon={filteredFields[key] ? <AiTwotoneFilter /> : <AiOutlineFilter />}
                                       type="text"
                                       size="small"
                                    />
                                 </Popover>
         ...
         <AddNewRowView setShowAddView={setShowAddView} showAddView={showAddView} fieldMap={fieldMap} onSave={onSave} />
      </RenderIf>
   );
};
